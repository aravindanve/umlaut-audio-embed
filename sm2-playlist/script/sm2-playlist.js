...
function initSm2Playlists () {
  ...
  Player = function(playerNode) {
    ...
    actions = {
      ...
      close: function () { // added by @aravindanve
        utils.css.add(dom.o, 'hidden');
        return actions.pause();
      },
  ...
  soundManager.onready(function() {
    ...
    if (nodes && nodes.length) {
      for (i=0, j=nodes.length; i<j; i++) {
        var player = new Player(nodes[i]);
        players.push(player);
        nodes[i].__player = player; // added by @aravindanve
      }
    }
